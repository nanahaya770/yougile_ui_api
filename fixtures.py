...
@pytest.fixture(
    params=["edge", "firefox", "chrome"], scope="module", autouse=True
)
...
def autorization(driver: WebDriver) -> None:
    ...
    login = driver.find_element(
        By.CSS_SELECTOR, "form > div:nth-child(1) input"
    )
    password = driver.find_element(
        By.CSS_SELECTOR, "form > div:nth-child(2) input"
    )

    assert type(LOGIN) is str, "логин не загрузился"
    assert type(PASSWORD) is str, "пароль не загрузился"
    login.send_keys(LOGIN)
    password.send_keys(PASSWORD)

    button_enter = driver.find_element(
        By.CSS_SELECTOR, "form div.justify-center"
    )
    button_enter.click()

    my_profile = driver.find_element(
        By.CSS_SELECTOR, "div.truncate.ml-6.text-14.leading-4"
    )
    assert (
        my_profile.text == MY_PROFILE_TEXT
    ), "не произошел переход на страницу личного кабинета"
...
def subtask_ids(driver: WebDriver) -> Generator[list[str], None, None]:
    ...
    Returns:
        Generator[list[str], None]: Генератор, дающий список идентификаторов
        созданных подзадач.
    """
    subtask_list = []
    URL = f"{BASE_URL}/tasks"
    for num_subtask_id in range(1, NUMBER_OF_SUBTASKS + 1):
        body = {"title": f"подзадача {num_subtask_id}"}
    ...
    for subtask_id in subtask_list:
        ...
        body_for_del = {"deleted": True}
        response = requests.put(URL, headers=HEADERS, json=body_for_del)
        assert (
            response.status_code == 200
        ), f"подзадача {subtask_id} не удалена"


@pytest.fixture(scope="module", autouse=True)
def create_task(
    subtask_ids: Generator[list[str], None],
) -> Generator[None, None]:
    """
    Фикстура создаёт основную задачу с подзадачами и удаляет её по завершении,
    принимает список подзадач

    Args:
        subtask_ids (Generator[list[str], None]):
         Генератор со списком id подзадач.

    Returns:
        Generator[None, None]: Генератор без значения;
        ресурс удаляется в teardown.
    """
    URL = f"{BASE_URL}/tasks"
    body_for_task = {
        "title": TITLE,
...
def completed_subtasks(subtask_ids: Generator[list[str], None]) -> None:
    """
    Фикстура переводит подзадачи в статус выполнено,
    принимает фикстуру subtask_ids
    ...
    for subtask_id in subtask_ids:
        ...
        assert (
            response.status_code == 200
        ), "подзадача не переведена в статус - выполнено"
